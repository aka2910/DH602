...
def tile(img, mask, image_id):
    ...
    if len(img) < N:
        mask = np.pad(
            mask, [[0, N - len(img)], [0, 0], [0, 0], [0, 0]], constant_values=0
        )
        img = np.pad(
            img, [[0, N - len(img)], [0, 0], [0, 0], [0, 0]], constant_values=255
        )
...
if __name__ == "__main__":
    ...
    for i in tqdm(range(len(train_df))):
        img = skimage.io.MultiImage(
            os.path.join(TRAIN, train_df["image_id"].iloc[i] + ".tiff")
        )[-1]
        mask = skimage.io.MultiImage(
            os.path.join(MASKS, train_df["image_id"].iloc[i] + "_mask.tiff")
        )[-1]
